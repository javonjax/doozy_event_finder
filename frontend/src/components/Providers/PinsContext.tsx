...
export const PinsContext = createContext<PinsContextProvider | undefined>(
  undefined,
);

// Tracks and updates pinned events for the current user.
export const PinsProvider = ({ children }: { children: React.ReactNode }) => {
  const authContext: AuthContextProvider | undefined = useContext<
  ...
  const fetchPinnedEvents = async (): Promise<void> => {
    try {
      ...
      const sortedPinnedEvents: Array<PinnedEventData> | undefined =
        jsonRes?.pinnedEvents?.sort(
          (a, b) =>
  ...
  return (
    <PinsContext.Provider value={{ pinnedEvents, fetchPinnedEvents }}>
      {children}
