...
const Card = ({
  ...
  return (
    <NavLink
      className={clsx(
        "cursor-pointer min-h-[175px] border-2 h-[250px] w-[70%] md:w-[30%] md:h-full flex rounded-2xl overflow-hidden relative transform transition-all duration-400 hover:scale-105",
        colors,
      )}
    ...
    >
      <div
        className={`absolute top-2 left-0 ${path === "/popular" ? "text-orange-400" : ""}`}
      >
        {icon}
      </div>
      <label className="absolute bottom-4 right-4 text-[1.5rem] cursor-pointer">
        {label}
      </label>
    </NavLink>
  );
